...
if m:
    ...
    res2 = sess.post('https://mall-billing-system-geli.onrender.com/auth/login', data={'username': 'admin', 'password': 'Admin@2026'})
    print('Login Status with Header:', res2.status_code)
    
    # Test setting data instead of header
    sess2 = requests.Session()
    # Need to get a new token for the new session because CSRF is tied to the session cookie
    ...
    res4 = sess2.post('https://mall-billing-system-geli.onrender.com/auth/login', data={'username': 'admin', 'password': 'Admin@2026', 'csrf_token': m3.group(1)})
